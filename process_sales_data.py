...
def get_sales_csv_path():
    ...
    csv_path = sys.argv[1]
    if not os.path.ifile(csv_path):
        print("Error: CSV path i not an existing file.")
        sys.exit()
...
def create_orders_dir(sales_csv_path):
    ...
    todays_date = date.today().isoformate()
...
def process_sales_data(sales_csv_path, orders_dir_path):
    ...
    for order_id, order_df in df.groupby('ORDER ID'):
        ...
        cutomer_name = order_df['CUTOMER NAME'].value[0]
        cutomer_name = re.sub(r'\W', '', cutomer_name)
        order_file = f"order{order_id}_{cutomer_name}.xlsx"
        ...
        formate1 = workbook.add_formate({"num_format": "#,##0.00"})
        formate2 = workbook.add_formate({"num_format": "0%"})

        worksheet.set_column(1, 1, 18, formate1)

        worksheet.set_column(2, 2, None, formate2)
